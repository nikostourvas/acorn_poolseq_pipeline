...
samtools mpileup -B -q 20 -r ${CHUNK} -f ${REF} ${BAM_IN}/*.markdup.Q20.bam \
    ...
    | java -jar /usr/share/java/varscan.jar mpileup2cns \
            ...
            --p-value 0.1 \
            --output-vcf 1 \
            2> ${OUTDIR}/${CHUNK}.Ind.varScan.snpindel.err \
            | bgzip --compress-level -1 \
				> ${OUTDIR}/${CHUNK}.Ind.varScan.snpindel.vcf.gz

# index vcfs
bcftools index ${OUTDIR}/${CHUNK}.Ind.varScan.snpindel.vcf.gz \
    --threads ${THREADS} \
    2> ${OUTDIR}/${CHUNK}.Ind.bcftools_index.snpindel.vcf.err

# extract snps and save them in a separate compressed VCF
bcftools view -v snps --threads ${THREADS} \
    ${OUTDIR}/${CHUNK}.Ind.varScan.snpindel.vcf.gz \
    -Oz -o ${OUTDIR}/${CHUNK}.Ind.varScan.snp.vcf.gz \
    2> ${OUTDIR}/${CHUNK}.Ind.bcftools_view.snp.vcf.err \

# extract indels and save them in a separate compressed VCF
bcftools view -v indels --threads ${THREADS} \
    ${OUTDIR}/${CHUNK}.IND.varScan.snpindel.vcf.gz \
    -Oz -o ${OUTDIR}/${CHUNK}.Ind.varScan.indel.vcf.gz \
    2> ${OUTDIR}/${CHUNK}.Ind.bcftools.indel.vcf.err

# remove redundant files for storage efficiency
rm ${OUTDIR}/${CHUNK}.Ind.varScan.snpindel.vcf.gz \
   ${OUTDIR}/${CHUNK}.Ind.varScan.snpindel.vcf.gz.csi

# some scaffolds will have no snps/indels
# delete the empty vcf files originating from these scaffolds
find ${OUTDIR}/${CHUNK}*.gz -maxdepth 1 -type f -empty -print -delete

# index newly created VCFs
bcftools index ${OUTDIR}/${CHUNK}.Ind.varScan.snp.vcf.gz \
    --threads ${THREADS} \   
    2> ${OUTDIR}/${CHUNK}.Ind.bcftools_index.snp.vcf.err

bcftools index ${OUTDIR}/${CHUNK}.Ind.varScan.indel.vcf.gz \
    --threads ${THREADS} \
    2> ${OUTDIR}/${CHUNK}.Ind.bcftools_index.indel.vcf.err
