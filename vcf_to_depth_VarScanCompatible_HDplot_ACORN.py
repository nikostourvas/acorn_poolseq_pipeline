        ...
        with open(out_file, 'w') as OUTFILE:
            ...
            for line in INFILE:
			    ...
			    # print int(gen.split(':')[1].split(',')[0]) 
                            depth_a_of_ind[inds[gen_idx]] = int(gen.split(':')[4])   # for p316 where data is GT:AD:DP:GQ:PL
                            depth_b_of_ind[inds[gen_idx]] = int(gen.split(':')[5])   # for p316 
                    ...
                    if sum_a+sum_b > 0:
                        OUTFILE.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(contig, pos, locus_ID, sum_a, sum_b, float(sum_a)/(sum_a+sum_b), num_hets, num_samples))
                ...
                if line.startswith('##'):
                    ...
                    for gen_idx, gen in enumerate(genotypes):
                        if gen.split(':')[0] in ['1/0', '0/1']: # if het
                            depth_a = int(gen.split(':')[4].split(',')[0])
                            depth_b = int(gen.split(':')[4].split(',')[1])
