...
awk -F " " -v awk_working_directory="${PWD}" -v awk_populations="${POPULATIONS}" -v awk_set_k="${SET_K}" -v awk_genetic_dataset="${OUTPUT_DIR}/${IMPUTED_TABLE}" -v awk_environmental_factors="${ENV_DATA}" -v awk_fdr=${FDR} \
...
parallel --verbose -j 50 'Rscript /home/geneticsShare/acorn_poolseq_pipeline/LFMM_robust_Full_analysis_Part1.R {}' :::: ./par/LFMM_Full_Analysis_Part1_Parameters_${POPULATIONS}.txt
