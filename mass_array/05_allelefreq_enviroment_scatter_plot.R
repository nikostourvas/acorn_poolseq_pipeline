...
snp_positions_clean$chr <- as.integer(snp_positions_clean$chr)
snp_positions_clean <- na.omit(snp_positions_clean)
...
# After loading snp_positions_clean, expand combined bioclim entries
snp_positions_expanded <- snp_positions_clean |>
  separate_rows(bioclim, sep = ",")
...
dataset <- left_join(snp_positions_expanded, afs)
...
create_summary_plot <- function(
  ...
  filename_summary <- file.path(output_dir, "correlation_summary2.pdf")
