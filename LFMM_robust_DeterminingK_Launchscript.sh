...
MAX_K=$3 #The maximum value of K that you wish the script to analyse. The script will only run for multiple instance of K (1-maxK)
...
realpath ${OUTPUT_DIR}/${IMPUTED_TABLE/.txt/_Chunk??.txt} > LFMM_DeterminingK_Parameters_${POPULATIONS}_INTERMEDIATE.txt #Use the path of all 50 split genetic datasets as a scaffold to create the parameters file.
...
LFMM_DeterminingK_Parameters_${POPULATIONS}_INTERMEDIATE.txt > ./par/LFMM_DeterminingK_Parameters_Part1_${POPULATIONS}.txt
...
parallel --verbose -j 50 'Rscript /home/geneticsShare/acorn_poolseq_pipeline/LFMM_robust_DeterminingK_Part1.R {}' :::: ./par/LFMM_DeterminingK_Parameters_Part1_${POPULATIONS}.txt
