...
if [ ${TICKER2} -lt ${MAXTICKER2} ]; #Check whether a region file has been 'filled up' to the target size.
    then printf "%s\t%s\t%s\n" ${line} >> ${OUTDIR}/chunk${TICKER1}.bed; #If there is still space for more basepairs in the region file, add the next fragment, which is in practical terms a line from the input .bed file.
    ...
    printf "%s\t%s\t%s\n" ${line} >> ${OUTDIR}/chunk${TICKER1}.bed; #Instead of adding the next line of the .bed file to the 'full' region file, add it to an empty one.
