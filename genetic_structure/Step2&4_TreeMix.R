...
library(OptM)
library(plyr)
source("analysis/treemix_boot2/TreeMix/TreeMix_functions.R") #path to required functions for this analysis
...
folder <- file.path(path="analysis/treemix_boot2/test_migrations")                     #path to files of TreeMix replicates with different migration edges (m) to test
...
setwd("/home/rstudio/working/zurich_workshop/analysis/treemix_boot2/final_runs")                                             #folder with all TreeMix outputs from the final runs
maxLL("step3_1m_finalrun_", nt=30)                                         #first argument is stem of TreeMix output files, nt = number of runs
...
treemix.bootstrap("step3_1m_finalrun_1", out.file = "tmp",                 #stem of TreeMix files (as above) + number of run with highest likelihood and unique topology
                  phylip.file = "../step3_finalconstree.newick",           #consensus tree in newick format (from the bootstrap procedure generated with PHYLIP)    
...
treemix.drift(in.file = "step3_1m_finalrun_1",                             #pairwise matrix for drift estimates with specified order of pops 
              pop.order.color.file = "../col.txt") + 
              title("Drift")     

plot_resid("step3_1m_finalrun_1",                                          #pairwise matrix for residuals
           pop_order = "../col.txt") +
           title("Residuals")                         
dev.off()
...
GetMigrStats(input_stem="step3_1m_finalrun_", nt=30)                       #arguments as above, writes file "MS_and_stats.txt" into current directory
