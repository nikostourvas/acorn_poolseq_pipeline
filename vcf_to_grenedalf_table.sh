...
OUTPUT=${VCF/.vcf/} #Take the name of the input vcf without file type suffix.

N_SAMPLES=$(bcftools query -l ${VCF} | wc -l) #Extract the number of samples found in the vcf.
...
bcftools query -Hf '%CHROM\t%POS\t%REF\t%ALT[\t%RD][\t%AD]\n' ${VCF} \ #This line is responible for the data extraction
    | awk 'BEGIN{OFS=FS="\t"} 
        NR==1 {
            $1="chrom"; $2="pos"; $3="ref"; $4="alt"; #Name columns 
            for (i=5; i<=NF; i++){ #Change the format of the vcf fields to something that is easier to edit.
                gsub(/\[[^]]*\]/, "", $i); 
                gsub(/:RD$/, ".ref.cnt", $i); 
                gsub(/:AD$/, ".alt.cnt", $i)
            };
...
| sed -e 's/\t\t/\tNA\t/g' > ${OUTPUT}_intermediate.txt #Make sure that empty fields become 'NA'.
...
RD_POS=$((5+${n}));#Tell awk in which column to find an RD value
AD_POS=$((${RD_POS}+${N_SAMPLES}));#Tell awk in which column to find an AD value
FREQ_POS=$((${AD_POS}+${N_SAMPLES})); #Tell awk in which column to place the allele frequency it calculates.
awk -F '\t' -v awkRDpos="${RD_POS}" -v awkADpos="${AD_POS}" -v awkFREQpos="${FREQ_POS}" \ # Pass the variants we created on to awk
' OFS = "\t" {awkFREQvalue=$awkADpos/($awkRDpos+$awkADpos); print $0,  $awkFREQpos=awkFREQvalue }' ${OUTPUT}_intermediate.txt > \ #Calculate the allele frequency and place it in its corresponding column.
...
cut -f1,$((6+(2*${N_SAMPLES})))-$((6+(3*${N_SAMPLES}))) ${OUTPUT}_big.txt > ${OUTPUT}_small.txt #Extract only the columns that summarise the genomic position and all the allele frequencies.

head -n 1 ${OUTPUT}_big.txt | cut -f1,6-$((6+${N_SAMPLES})) | sed -e 's/P01-...-ACORN-BOKU-...-//g' |\ #Create a new header with only the sample numbers (ACORN specific)
