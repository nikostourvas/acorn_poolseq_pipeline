  ...
  #Plot the distribution of p-values. We plot them as PNGs, because the PDFs contained so many points that it took 5 minutes to load each one.
   ...
   # Summarise results and generate Manhattan plot at different significance thresholds
    for (x in 1:length(fdr.thres)) {
      ...
      if(NCOL(candidate) == 1) {
        write.table(t(candidate), paste("CandidatesOrdered_env", env.variable, "_K", set.k, "_q", q, ".csv", sep=""), sep=",", row.names=F, col.names=T, quote=F)
      } else {
        if(NCOL(candidate) > 1) {
          ...
          write.table(candidate, paste("CandidatesOrdered_env", env.variable, "_K", set.k, "_q", q, ".csv", sep=""), sep=",", row.names=T, col.names=F, quote=F)
        }
      }
    }
