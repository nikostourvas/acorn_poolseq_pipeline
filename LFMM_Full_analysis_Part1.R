...
dir.create(paste(dir.path, "/res/", populations, "/environment_" , sep=""), recursive=F)
for (i in 1:NCOL(X)) {
...
for (j in 1:NCOL(X)) {
  ...
  write.table(res, paste("LFMM_Zscores_", populations, "environment_", j, "_K", i, "_Chunk_", chunk, ".csv", sep=""), sep=",", row.names=F, col.names=T, quote=F) # save all information per SNP
  }
}
