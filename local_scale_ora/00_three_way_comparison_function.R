...
compare_three_tables <- function(
  ...
  if (is.null(n_windows)) {
    stop(
      "Error: n_windows (total universe size) must be provided as an argument"
    )
  }
  ...
  union_comparison <- tibble(
    ...
    prop_overlap_of_intersection = length(overlap_intersection_1_2_with_3) /
      length(overlap_1_2),
    prop_overlap_of_table3 = length(overlap_intersection_1_2_with_3) /
      length(indices3),
    prop_AB = length(overlap_intersection_1_2_with_3) /
      min(length(overlap_1_2), length(indices3)),
    pvalue = phyper(
      q = length(overlap_intersection_1_2_with_3) - 1,
  ...
  intersection_comparison_file <- file.path(
    output_dir,
    "intersection_comparison.csv"
  )
  ...
  overlap_plot <- ggplot(
    overlap_summary,
    aes(x = reorder(comparison, count), y = count)
  ) +
  ...
  ggsave(
    file.path(output_dir, "overlap_counts_barplot.png"),
    overlap_plot,
    width = 10,
    height = 6,
    dpi = 300
  ...
  pairwise_plot <- ggplot(
    ...
    aes(x = set_A, y = set_B, fill = prop_AB)
  ) +
    geom_tile(color = "white", size = 1) +
    geom_text(
      aes(label = paste0(round(prop_AB * 100, 1), "%")),
      color = "white",
      size = 4
    ) +
  ...
  ggsave(
    file.path(output_dir, "pairwise_overlap_heatmap.png"),
    pairwise_plot,
    width = 8,
    height = 6,
    dpi = 300
