...
#Create the 2D histogram plot:
  ...
  px <- ggplot(dataset, aes(x=alt_ind)) + geom_histogram() + theme_void()
  py <- ggplot(dataset, aes(x=alt_pool)) + geom_histogram() + theme_void() +
                coord_flip()
...
rawdiffplot <- function(dataset){
  p <- ggplot(dataset, aes(x=alt_pool - alt_ind, y=alt_pool)) +
       geom_bin2d(bins=100) + 
       scale_fill_viridis(trans='log10') +
       theme_minimal() + 
       labs(x="Raw AF differences (Pool AF - Ind AF)",
            y="Pool-Seq Allele Frequency (AF)",
            fill="Log10 Density") + 
       theme(legend.position = c(1.2, 1.15))
  #Create marginal histograms:
  px <- ggplot(dataset, aes(x=alt_pool - alt_ind)) + 
          geom_histogram(bins=100) + theme_void()
  py <- ggplot(dataset, aes(x=alt_ind)) + geom_histogram() + theme_void() +
                coord_flip()
...
AF_barplot <- function(dataset, title="", bins=40){
...
poplist <- poplist
for (i in 1:2){
  poplist[[i]][,c(2,3)][poplist[[i]][,c(2,3)] < 0.05] <- NA
