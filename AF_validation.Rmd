...
error_stats <- function(ind_freq, pool_freq){
  ...
  out <- paste("\n", r, Rsquared, rmse, mae, mape_res, sep="\n")
  return(out)
}
...
```{r}
# Pool data
pooldata_markdup <- read.table("/data/genetics_tmp/mapping_tests/results_NT/duplicate_filtering_AF_validation/VCF/validationIndelFilteredSNPs_Biallelic_markdup_RD30.vcf_AlleleFrequencyTable.txt", sep = "\t", header=TRUE)
pool305_markdup <- pooldata_markdup %>% 
                   select(chrom_pos, X305_markdup)
colnames(pool305_markdup) <- c("site", "alt_pool")
...
```{r}
# Pool data
pooldata_dedup <- read.table("/data/genetics_tmp/mapping_tests/results_NT/duplicate_filtering_AF_validation/VCF/validationIndelFilteredSNPs_Biallelic_dedup_NOmarkdup_RD30.vcf_AlleleFrequencyTable.txt", sep = "\t", header=TRUE)
pool305_dedup <- pooldata_dedup %>% 
                   select(chrom_pos, X305_dedup_NOmarkdup)
colnames(pool305_dedup) <- c("site", "alt_pool")
...
```{r}
# Pool data
pooldata_dedup_markdup <- read.table("/data/genetics_tmp/mapping_tests/results_NT/duplicate_filtering_AF_validation/VCF/validationIndelFilteredSNPs_Biallelic_dedup_markdup_RD30.vcf_AlleleFrequencyTable.txt", sep = "\t", header=TRUE)
pool305_dedup_markdup <- pooldata_dedup_markdup %>% 
                   select(chrom_pos, X305_dedup_markdup)
colnames(pool305_dedup_markdup) <- c("site", "alt_pool")
