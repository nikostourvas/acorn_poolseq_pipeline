...
#Create the 2D histogram plot:
  ...
  #Create marginal histograms:
  px <- ggplot(dataset, aes(x=maf_ind)) + geom_histogram(bins=20) + theme_void()
  py <- ggplot(dataset, aes(x=maf_pool)) + geom_histogram(bins=20) + theme_void() +
                coord_flip()
...
histALTfreq <- function(dataset, alt_ind, alt_pool, title=""){
  ggplot(dataset) +
  geom_histogram(aes(x=alt_ind, after_stat(density)), bins = 40,
