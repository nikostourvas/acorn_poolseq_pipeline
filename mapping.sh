...
bwa mem -R ${RG} -M -t 2 ${REF} ${FORWARD} ${REVERSE} > ${OUTPUT}.raw.sam

# -@ how many cores to use PER SAMPLE
samtools view -@ 2 -b ${OUTPUT}.raw.sam > ${OUTPUT}.raw.bam
rm ${OUTPUT}.raw.sam
# sort reads in the BAM according to their names so that pairs are placed one below the other
samtools sort -n -@ 2 ${OUTPUT}.raw.bam > ${OUTPUT}.name.sort.bam
rm ${OUTPUT}.raw.bam
