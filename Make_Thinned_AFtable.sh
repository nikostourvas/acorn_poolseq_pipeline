...
# 2024.05.23
# A modification of the script "Make_Frequency_Table.sh" that thins SNPs before creating the AF table
...
THINNED_VCF=${OUTPUT}_Thinned_${THINNING_THRESHOLD}bp.recode.vcf
vcftools --vcf ${VCF} --thin ${THINNING_THRESHOLD} --recode --recode-INFO-all --out ${THINNED_VCF/.recode.vcf/} &&
...
| sed -e 's/\t\t/\tNA\t/g' > ${OUTPUT}_Thinned_${THINNING_THRESHOLD}bp_intermediate.txt
